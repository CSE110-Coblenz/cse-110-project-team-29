...
export class EndOfGameScreenView implements View {
  ...
  constructor(stageWidth: number, stageHeight: number) {
    ...
    // button
    const btnY = card.y() + card.height() - 130;
    const mkBtn = (x: number, label: string) => {
      ...
      const bg = new Konva.Rect({
        ...
        stroke: label.includes("Play") ? "#8b4a2a" : "#573233",
        strokeWidth: 1,
      });
    ...
    const btnRetry = mkBtn(card.x() + card.width() - 460, "Play Again");
    const btnExit = mkBtn(card.x() + card.width() - 230, "Exit to Title");
  ...
  onRetryClick(cb: ClickHandler) {
    this.onRetry = cb;
  }
  onExitClick(cb: ClickHandler) {
    this.onExit = cb;
  }
  ...
  render(data: EndGameData, stageWidth: number, stageHeight: number) {
    ...
    // ring
    const cx = stageWidth * 0.33;
    const cy = stageHeight * 0.4;
    const outerR = 120,
      innerR = 80;

    // inside ring
    const baseArc = new Konva.Arc({
      x: cx,
      y: cy,
      innerRadius: innerR,
      outerRadius: outerR,
    ...
    this.donutLayer.add(baseArc);

    // targer percent and angle
    const targetPercent = Math.max(0, Math.min(100, data.accuracy));
    const targetAngle = (targetPercent / 100) * 360;

    // progressArc form 0-100
    ...
    this.donutLayer.add(progressArc);

    // percent text from 0-100
    const percentText = new Konva.Text({
      x: cx - 60,
      y: cy - 24,
      width: 120,
      align: "center",
    ...
    this.donutLayer.add(percentText);
    ...
    const tween = new Konva.Tween({
      ...
      onUpdate: () => {
        const currentAngle = progressArc.angle();
        const currentPercent = Math.round((currentAngle / 360) * 100);
    ...
    // bottom table
    const baseX = stageWidth * 0.16 ,
      baseY = stageHeight * 0.86;
    const chartW = stageWidth * 0.70,
      chartH = 220;
    const n = data.performanceByConcept.length,
      gap2 = 90;
    ...
    data.performanceByConcept.forEach((c, i) => {
      ...
      const rect = new Konva.Rect({
        ...
        fillLinearGradientColorStops: [0, "#ffb703", 1, "#d08a00"],
        stroke: "#5a3b1e",
        strokeWidth: 1,
        cornerRadius: 8,
      });
